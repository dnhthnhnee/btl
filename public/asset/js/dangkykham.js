...
async function addEventToFirstAvailableSubcollection(db, subCollections, obj) {
    ...
    const tmp = doc(db, "LSK", "số lượng");
    const doctmp = await getDoc(tmp);
    for (let i = 0; i < subCollections.length; i++) {
        ...
        if (!docSnap.exists()) {
            ...
            const sl = doctmp.data()["so luong"] + 1;
            const lsk = doc(db, "LSK", sl.toString());
            const docData = {
                "Appt": temp.getAt('Appt'),
                "ID bac si": subCollections[i],
                "ID benh nhan": temp.getAt('Email')
            }
            await setDoc(lsk, docData);
            await setDoc(tmp, { "so luong": doctmp.data()["so luong"] + 1 });
            return;
        }
    }
